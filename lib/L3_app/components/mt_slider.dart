...
class MTSlider extends StatefulWidget {
  MTSlider({
    ...
    this.tooltip,
    this.trackBar = const MTSliderTrackBar(),
    this.handlerAnimation = const MTSliderHandlerAnimation(),
    this.selectByTap = true,
    this.step = const MTSliderStep(),
    this.hatchMark,
  ...
  final MTSliderHandler? handler;
  final MTSliderHandler? rightHandler;
  ...
  final List<MTSliderFixedValue>? fixedValues;
  final bool rangeSlider;
  final bool rtl;
  final bool jump;
  final bool selectByTap;
  final List<MTSliderIgnoreSteps> ignoreSteps;
  final bool disabled;
  final double? touchSize;
  final bool visibleTouchArea;
  final double minimumDistance;
  final double maximumDistance;
  final MTSliderHandlerAnimation handlerAnimation;
  final MTSliderTooltip? tooltip;
  final MTSliderTrackBar trackBar;
  final MTSliderStep step;
  final MTSliderHatchMark? hatchMark;
  ...
  @override
  State createState() => _MTSliderState();
}

class MTSliderFixedValue {
  MTSliderFixedValue({required this.percent, required this.value}) : assert(percent >= 0 && percent <= 100);
...
class MTSliderHandler {
  MTSliderHandler({this.child, this.decoration, this.foregroundDecoration, this.transform, this.disabled = false, this.opacity = 1});
...
class MTSliderHandlerAnimation {
  const MTSliderHandlerAnimation(
...
class MTSliderHatchMark {
  MTSliderHatchMark(
      ...
      this.smallLine,
      this.bigLine,
      this.linesAlignment = MTSliderHatchMarkAlignment.right,
      this.labelBox,
      this.displayLines})
  ...
  List<MTSliderHatchMarkLabel>? labels;
  MTSliderSizedBox? smallLine;
  MTSliderSizedBox? bigLine;
  MTSliderSizedBox? labelBox;
  MTSliderHatchMarkAlignment linesAlignment;
  bool? displayLines;
}

enum MTSliderHatchMarkAlignment { left, right }

class MTSliderHatchMarkLabel {
...
class MTSliderIgnoreSteps {
  MTSliderIgnoreSteps({this.from, this.to}) : assert(from != null && to != null && from <= to);

  final double? from;
  final double? to;
}

class MTSliderRangeStep {
  MTSliderRangeStep({
    this.from,
    this.to,
    this.step,
...
class MTSliderSizedBox {
  const MTSliderSizedBox({this.decoration, this.foregroundDecoration, this.transform, required this.height, required this.width})
...
class MTSliderStep {
  ...
  final List<MTSliderRangeStep>? rangeList;
}

class MTSliderTooltip {
  MTSliderTooltip({
    this.custom,
    this.format,
    this.textStyle,
  ...
  MTSliderTooltipDirection? direction;

  MTSliderTooltipPositionOffset? positionOffset;
}

class MTSliderTooltipBox {
  const MTSliderTooltipBox({this.decoration, this.foregroundDecoration, this.transform});
...
enum MTSliderTooltipDirection { top, left, right }

class MTSliderTooltipPositionOffset {
  MTSliderTooltipPositionOffset({this.top, this.left, this.right, this.bottom});
...
class MTSliderTrackBar {
  const MTSliderTrackBar({
...
class _MTSliderState extends State<MTSlider> with TickerProviderStateMixin {
  ...
  late MTSliderTooltip _tooltipData;
  ...
  double? _widgetMin;
  double? _widgetMax;
  List<MTSliderIgnoreSteps> _ignoreSteps = [];
  final List<MTSliderFixedValue> _fixedValues = [];
  ...
  void initMethod() {
    ...
    switch (_tooltipData.direction) {
      case MTSliderTooltipDirection.top:
        animationFinish = const Offset(0, -1);
        break;
      case MTSliderTooltipDirection.left:
        animationFinish = const Offset(-1, 0);
        break;
      case MTSliderTooltipDirection.right:
        animationFinish = const Offset(1, 0);
        break;
      default:
  ...
  void _checkRangeStep(double realValue) {
    ...
    if (widget.step.rangeList != null) {
      for (MTSliderRangeStep rangeStep in widget.step.rangeList!) {
  ...
  void _drawHatchMark() {
    ...
    final MTSliderHatchMark hatchMark = MTSliderHatchMark();
    ...
    hatchMark.smallLine =
        widget.hatchMark!.smallLine ?? const MTSliderSizedBox(height: 5, width: 1, decoration: BoxDecoration(color: Colors.black45));
    hatchMark.bigLine = widget.hatchMark!.bigLine ?? const MTSliderSizedBox(height: 9, width: 2, decoration: BoxDecoration(color: Colors.black45));
    hatchMark.labelBox = widget.hatchMark!.labelBox ?? const MTSliderSizedBox(height: 50, width: 50);
    ...
    if (hatchMark.displayLines!) {
      ...
      if (widget.axis == Axis.horizontal) {
        if (hatchMark.linesAlignment == MTSliderHatchMarkAlignment.left) {
      ...
      for (int p = 0; p <= percent; p++) {
        MTSliderSizedBox? barLineBox = hatchMark.smallLine;
...
//        left = (p * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
          ...
          if (hatchMark.linesAlignment == MTSliderHatchMarkAlignment.right) {
    ...
    if (hatchMark.labels != null && hatchMark.labels!.isNotEmpty) {
      ...
      int? tr;
      for (MTSliderHatchMarkLabel markLabel in hatchMark.labels!) {
...
//          left = (tr * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
  ...
  double? _findBiggestIgnorePoint({dynamic ignoreBeyondBoundaries = false}) {
    ...
    for (MTSliderIgnoreSteps steps in _ignoreSteps) {
  ...
  void _generateHandler() {
    /*Right Handler Data*/
    final MTSliderHandler inputRightHandler = widget.rightHandler ?? MTSliderHandler();
  ...
  void _setParameters() {
    ...
    if (widget.fixedValues != null && widget.fixedValues!.isNotEmpty) {
      ...
      for (MTSliderFixedValue fixedValue in widget.fixedValues!) {
        ...
        for (MTSliderFixedValue fixedValue in widget.fixedValues!) {
          if (fixedValue.percent == fixedPercent.toInt()) {
            ...
            if (fixedPercent > lowerIgnoreBound + 1 || lowerIgnoreBound == 0) {
              ...
              _ignoreSteps.add(MTSliderIgnoreSteps(from: lowerIgnoreBound, to: upperIgnoreBound));
            }
            lowerIgnoreBound = fixedPercent;
            break;
          }
        }
        _fixedValues.add(MTSliderFixedValue(percent: fixedPercent.toInt(), value: fValue));
      ...
      if (!fixedV.contains(100)) {
        _ignoreSteps.add(MTSliderIgnoreSteps(from: biggestPoint! + 1, to: 101));
      }
    } else {
    ...
    final widgetTooltip = widget.tooltip ?? MTSliderTooltip();

    _tooltipData = MTSliderTooltip();
    _tooltipData.boxStyle = widgetTooltip.boxStyle ??
        MTSliderTooltipBox(decoration: BoxDecoration(border: Border.all(color: Colors.black12, width: 0.5), color: const Color(0xffffffff)));
    ...
    _tooltipData.direction = widgetTooltip.direction ?? MTSliderTooltipDirection.top;
  ...
  Positioned _tooltip({String? side, dynamic value, double? opacity, Animation? animation}) {
    ...
    if (_tooltipData.direction == MTSliderTooltipDirection.top) {
...
class _MakeHandler extends StatelessWidget {
  ...
  final double? height;
  final GlobalKey? id;
  final MTSliderHandler? handlerData;
  final bool? visibleTouchArea;
  ...
  Widget build(BuildContext context) {
    ...
    final MTSliderHandler handler = handlerData ?? MTSliderHandler();
